...
<div id="m-poll">
<div class="head" style="margin-bottom:9px;padding-bottom:0">Голосование</div>

<?php 
if ($params->get('show_poll_title'))  
    echo "<h4>".$poll->title."</h4>"; 

 if ($display_poll) { ?>
    <form action="<?php echo JRoute::_('index.php');?>" method="post" id="poll_vote_<?php echo $poll->id;?>">
    <!--<form action="./component/acepolls/poll/1-otkuda-vy-uznali-o-nashem-sajte" method="post" id="poll_vote_<?php //echo $poll->id;?>">-->
        <?php 
        ...
        <?
            ...
            if ($params->get('show_msg')) {
                echo '<div id="mod_poll_messages_'.$poll->id.'" style="margin:5px;margin-top:13px">'.JText::_($msg);
        ...
        <div style="padding:2px" id="poll_buttons_<?php echo $poll->id;?>" >
            <input style="width:160px;padding-bottom:2px" type="submit" id="submit_vote_<?php echo $poll->id; ?>" name="task_button" class="votebutton addtocart-button" value="Голосовать" <?php echo $disabled; ?> />
        </div>
        <div id="poll_loading_<?php echo $poll->id;?>" style="display:none">
                <?php echo JText::_('MOD_ACEPOLLS_PROCESSING'); ?>
        </div>
        <div>
...
<a class="poll_result_link" href="<?php// echo JRoute::_('index.php?option=com_acepolls&view=poll&id='.$slug.$itemid); ?>" title=""><?php// echo JText::_('MOD_ACEPOLLS_VIEW_DETAILS'); ?></a><br />
<?php //endif; ?>
<?php //if ($params->get('show_rel_article')) : ?>
<a class="poll_result_link" target="<?php// echo $params->get('rel_article_window'); ?>" href="<?php //echo JRoute::_($params->get('rel_article')); ?>" title="">
...
function radioAction1(){
$('#wrap_voteid1').children('.radio').toggleClass('action');
}
});
</script> -->
...
jQuery('#poll_vote_1 div').click(function(){
jQuery('.radio').removeClass('active');
jQuery(this).find('.radio').addClass('active');
jQuery(this).find('.radio').next().attr('checked','checked');
});
if(!navigator.userAgent.match(/iPad|iPhone|Android|IEMobile|BlackBerry/i)){
//jQuery('.delivery-sel').selectBox();
}
});
</script>