...
abstract class modArticlesCategoryHelper
{
	public static function getList(&$params)
	{
		...
		foreach ($items as &$item)
		{
			...
			if ($item->catid) {
				...
				$item->displayCategoryTitle = $show_category ? '<a title="'.$item->category_title.'" href="'.$item->displayCategoryLink.'">'.$item->category_title.'</a>' : '';
			}
			else {
			...
			//$item->displayIntrotext = /*$show_introtext ?*/ self::truncate($item->introtext, $introtext_limit); // : '';
                        $item->introtext = self::_cleanIntrotext($item->introtext);
                        $item->displayIntrotext = mb_substr($item->introtext, 0, 110);
	...
	public static function _cleanIntrotext($introtext)
	{
		...
		$introtext = str_replace('</p>', '', $introtext);
		//$introtext = strip_tags($introtext, '<a><em><strong>');comment oleg
	...
	*/
	public static function truncate($html, $maxLength = 0)
	{
		...
		$output = preg_replace("/<\/p>$/","&hellip;</p>",$output);//$output.'&hellip;';
        return $output;
	}
