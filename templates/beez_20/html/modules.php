...
<?php if ($module->showtitle) { ?> <h<?php echo $headerLevel; ?>><span class="backh"><span class="backh2"><span class="backh3"><?php echo $module->title; ?></span></span></span></h<?php echo $headerLevel; ?>>
...
function modChrome_beezTabs($module, $params, $attribs)
{
	...
	if ($modulecount == 1) {
		...
		foreach($modules as $rendermodule) {
			echo '<li class="tab"><a title="" href="#" id="link_'.$rendermodule->id.'" class="linkopen" onclick="tabshow(\'module_'. $rendermodule->id.'\');return false">'.$rendermodule->title.'</a></li>';
		}
		echo '</ul>';
		$counter=0;
		// modulecontent
		foreach($modules as $rendermodule) {
			...
			{
			echo '<a title="" href="#" class="unseen" onclick="nexttab(\'module_'. $rendermodule->id.'\');return false;" id="next_'.$rendermodule->id.'">'.JText::_('TPL_BEEZ2_NEXTTAB').'</a>';
			}
			echo '</div>';
		}
		$modulecount--;
