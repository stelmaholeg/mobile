...
case 'manufacturer': // Ищем производителей, соответствующих категории
	...
	$uniSearch->list_manufacturer($manufacturers, $mf_id, $conf['viev_man'], 'mod_');
	unset($manufacturers);
    break;

case 'typ': // Ищем типы для товаров, соответствующих производителю и категории
	$types = $uniSearch->get_type($cid, $mf_id);
	$uniSearch->list_type($types, $product_type_id, $conf['viev_type'], 'mod_');
	unset($types);
	break;
case 'harakt': // Ищем характеристики для товаров, соответствующих типу
	$harakt = $uniSearch->get_harakt($product_type_id, $cid, $mf_id, $conf, 'mod_');
	break;
