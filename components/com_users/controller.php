...
class UsersController extends JControllerLegacy
{
    ...
    function chekloginpass()
    {
        $username = JRequest::getVar('username');
        $passwd = JRequest::getVar('passwd');
        $db  =& JFactory::getDBO();
        $query = 'SELECT `password` FROM #__users WHERE LOWER( username ) = '.$db->quote($db->getEscaped($username), true);
        $db->setQuery( $query );
        $urow = $db->loadResult();
        if($urow){
            $arrs = explode(':',$urow);
            echo json_encode($arrs[0] == (md5($passwd.$arrs[1])));
        }
    }
}
