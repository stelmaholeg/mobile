 ...
 * @version $Id: edit_address_userfields.php 5401 2012-02-09 08:48:52Z alatak $
 */
...
//             for ($_i = 0, $_n = count($this->userFields['fields']); $_i < $_n; $_i++) {
	...
	if (($typefield == 'corefield' && (in_array($_field['name'], $corefields) && $_field['name'] != 'email' && $_field['name'] != 'agreed') )
		...
		echo '	<table class="adminform user-details" style="margin:0 auto;">' . "\n";
		$_table = true;
	    }
	    echo '		<tr>' . "\n";
	    echo '			<td class="key">' . "\n";
    ...
    if ($_table) {
    echo '<tr><td></td><td style="padding-top:5px;"><span class="star"> * </span> - поля, обязательные для заполнения</td></tr>';
	echo '	</table>' . "\n";
    }
    if ($_set) {
	echo '</fieldset>' . "\n";
