		...
		if ($result == 0) {
			if ($product_id == $favorite_id && $mode=="fav_add" && $user_id > 0) {
				...
				if ( $user_id > 0 ) {
					...
					$addtofavorites .= '<input type="submit" style="min-width:auto;" class="addtofav_button" style="min-width:auto;" value="В лист желаний&nbsp;" name="addtofavorites" title="'.JText::_('VM_ADD_TO_FAVORITES').'&nbsp;" /><input type="hidden" name="favorite_id" value="'.$product_id.'" /><input type="hidden" name="user_id" value="'.$my->id.'" /><input type="hidden" name="mode" value="fav_add" /></form></div>';
				}
			}
		}
		if ($result > 0 ){ 
