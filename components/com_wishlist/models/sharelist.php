 ...
 */
class FavoritesModelSharelist extends JModel{
	...
	*/
	...
	public function getData(){		
		...
		if (empty( $this->_data )){
			...
			$db =& JFactory::getDBO();
			$query = 'SELECT u.name, fs.share_date, fs.share_title, fs.share_desc, fs.isWishList, fp.fav_id, fp.product_id, fp.product_qty, fp.user_id, fp.fav_date FROM #__users u, #__virtuemart_favorites_sh fs, #__virtuemart_favorites fp WHERE ' . (isset($recordSet->published)?'`published`':'1') . ' = 1 AND fp.user_id=' . $user_id .' AND fp.user_id=fs.user_id AND fp.user_id=u.id GROUP BY fp.product_id ORDER BY fp.fav_date DESC';
