 ...
 */
class FavoritesModelProductlist extends JModel{
	/**
	...
	*/
	...
	private function _buildQueryWhere() {
		...
		$allowedSearch = explode(',', 'virtuemart_product_id,product_sku,product_name'); // array('id', 'ordering', 'published'); 
		$where = ' WHERE (0=1) ';
		foreach($allowedSearch as $field){
			if (!$field) return '';
			$where .= " OR (prod.`$field` LIKE '%" . addSlashes($search) . "%') ";
		}
	    return $where;
	}
	...
	public function getTotal(){
		$db =& JFactory::getDBO();
		$db->setQuery( 'SELECT COUNT(*) FROM `#__virtuemart_products` prod ' . $this->_buildQueryWhere() );
