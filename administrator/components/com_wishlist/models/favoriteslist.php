...
class FavoritesModelFavoriteslist extends JModel{
	...
	*/
	...
	private function _buildQuery(){
		...
		$query = 'SELECT fav.fav_id, fav.product_id, prod_lng.product_name, fav.product_qty, fav.user_id, usr.name, fav.fav_date FROM `#__virtuemart_favorites` fav join `#__virtuemart_products` prod on fav.product_id = prod.virtuemart_product_id left join `#__virtuemart_products_'.$lang.'` prod_lng on fav.product_id = prod_lng.virtuemart_product_id left join `#__users` usr on fav.user_id = usr.id ' . $this->_buildQueryWhere() . $this->_buildQueryOrderBy();
		return $query;
	}
	...
	public function getTotal(){
		...
		$db =& JFactory::getDBO();
		$db->setQuery( 'SELECT COUNT(*), fav.fav_id, fav.product_id, prod_lng.product_name, fav.product_qty, fav.user_id, usr.name, fav.fav_date FROM `#__virtuemart_favorites` fav join `#__virtuemart_products` prod on fav.product_id = prod.virtuemart_product_id left join `#__virtuemart_products_'.$lang.'` prod_lng on fav.product_id = prod_lng.virtuemart_product_id left join `#__users` usr on fav.user_id = usr.id ' . $this->_buildQueryWhere() );
