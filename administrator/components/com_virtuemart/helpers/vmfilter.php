...
if(!is_null(vmFilter::$C)){$reC = vmFilter::$C;}
vmFilter::$C = $C;
$S = is_array($S) ? $S : vmFilter::$hl_spec($S);
if(!is_null(vmFilter::$S)){$reS = vmFilter::$S;}
vmFilter::$S = $S;
...
if($C['cdata'] or $C['comment']){$t = preg_replace_callback('`<!(?:(?:--.*?--)|(?:\[CDATA\[.*?\]\]))>`sm', 'vmFilter::hl_cmtcd', $t);}
$t = preg_replace_callback('`&amp;([A-Za-z][A-Za-z0-9]{1,30}|#(?:[0-9]{1,8}|[Xx][0-9A-Fa-f]{1,7}));`', 'vmFilter::hl_ent', str_replace('&', '&amp;', $t));
if($C['unique_ids'] && is_null(vmFilter::$hl_Ids)){vmFilter::$hl_Ids = array();}
...
unset($C, $e);
if(isset($reC)){vmFilter::$C = $reC;}
if(isset($reS)){vmFilter::$S = $reS;}
return $t;
// eof
}
...
if(vmFilter::$C['direct_list_nest']){$cS['ol'] = $cS['ul'] += array('ol'=>1, 'ul'=>1);}
...
for($i = count(($t = explode(';', $t))); --$i>=0;){
 ...
 foreach(explode(',', $a) as $v){
  ...
  if(isset($y[$x]['match']) && !vmFilter::hl_regex($y[$x]['match'])){unset($y[$x]['match']);}
  if(isset($y[$x]['nomatch']) && !vmFilter::hl_regex($y[$x]['nomatch'])){unset($y[$x]['nomatch']);}
 }
...
foreach($aA as $k=>$v){
 if(((isset($C['deny_attribute']['*']) ? isset($C['deny_attribute'][$k]) : !isset($C['deny_attribute'][$k])) or isset($rl[$k])) && ((!isset($rl['n'][$k]) && !isset($rl['n']['*'])) or isset($rl[$k])) && (isset($aN[$k][$e]) or (isset($aNU[$k]) && !isset($aNU[$k][$e])))){
  ...
  if(isset($rl[$k]) && is_array($rl[$k]) && ($v = vmFilter::hl_attrval($v, $rl[$k])) === 0){continue;}
  $a[$k] = str_replace('"', '&quot;', $v);
 }
}
...
if($C['unique_ids'] && isset($a['id'])){
 if(!preg_match('`^[A-Za-z][A-Za-z0-9_\-.:]*$`', ($id = $a['id'])) or (!is_null(vmFilter::$hl_Ids[$id]) && $C['unique_ids'] == 1)){unset($a['id']);
 }else{
  while(!is_null(vmFilter::$hl_Ids[$id])){$id = $C['unique_ids']. $id;}
