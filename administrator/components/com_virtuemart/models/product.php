...
// 			vmdebug('product allows following orderingFields ',$this->_validOrderingFieldName);
	...
	function sortSearchListQuery($onlyPublished=true,$virtuemart_category_id = false, $group=false,$nbrReturnProducts=false){
		...
		if ( !empty($this->keyword) and $this->keyword !=='' and $group ===false) {
			$keyword = '"%' . trim(preg_replace('/\s+/', '%', $this->_db->getEscaped($this->keyword,true)), '%') . '%"'; // added
			//$keyword = '"%' . $this->_db->getEscaped($this->keyword, true) . '%"'; //commented
		...
		if($group){
			...
			switch ($group) {
                ...
                case 'topten':
					...
					$this->filter_order_Dir = 'DESC';
                case 'discounts':
                    $where[] = 'pp.`product_override_price` <> 0';
                    break;
			}
		...
		if ($joinCategory == true) {
			$joinedTables .= ' LEFT JOIN `#__virtuemart_product_categories` ON p.`virtuemart_product_id` = `#__virtuemart_product_categories`.`virtuemart_product_id`
			 ...
			 LEFT JOIN `xdno1_virtuemart_product_customfields` as cf ON cf.`virtuemart_product_id` = p.`virtuemart_product_id`';
		}
		if ($joinMf == true) {
		...
		echo '<div style="display:none;">' . json_encode($whereString) . '</div>';
		
        $needcut = false;
        if((isset($_REQUEST['famount']) || isset($_REQUEST['tamount']) || isset($_POST['f'])) && !$nbrReturnProducts){
            $needcut = true;
            $product_ids =  $this->exeSortSearchListQuery(2, $select, $joinedTables, $whereString, $groupBy, $orderBy, $this->filter_order_Dir, -1);
        } else {
        ...
        if($needcut){
            $product_ids_imp = implode(',',$product_ids);

            $where2 = array();
            $innerdb = JFactory::getDbo();
            if(isset($_POST['f']) && is_array($_POST['f'])){
                foreach($_POST['f'] as $custid => $custval){   // {218->array('asus','acer')}
                    $custvalvals = array();
                    foreach($custval as $custind => $custvalval){  // {0->asus, 1->acer}
                        $custvalvals[] = '"' . $custvalval . '"';
                    }
                    $custvalvals = implode(',',$custvalvals);
                    $query = "SELECT `virtuemart_product_id` FROM `xdno1_virtuemart_product_customfields`
                               WHERE virtuemart_custom_id = ".$custid."
                                     AND custom_value in (" . $custvalvals . ")
                                     AND virtuemart_product_id in (". $product_ids_imp . ")";
                    $innerdb->setQuery($query);
                    $product_ids = $innerdb->loadResultArray();
                    $product_ids_imp = implode(',',$product_ids);
                }
            }

            echo "<div class='queries' style='display:none;'>";
            echo "BEFOREPRICE:" . $product_ids_imp . "<br/>";
            echo "famount = " . $_REQUEST['famount'] . "<br/>";
            echo "tamount = " . $_REQUEST['tamount'] . "<br/>";

            if(isset($_REQUEST['famount']) && isset($_REQUEST['tamount'])){
                $query = "SELECT `virtuemart_product_id` FROM `xdno1_virtuemart_product_prices`
                           WHERE virtuemart_product_id in (". $product_ids_imp . ")
                             AND product_price BETWEEN ".$_REQUEST['famount']." AND " . $_REQUEST['tamount'];
                $innerdb->setQuery($query);
                $product_ids = $innerdb->loadResultArray();
                echo "Q:" . $query . "<br/>";
            ...
            echo "AFTERPRICE:" . $product_ids_imp . "<br/>";

            $limits = $this->setPaginationLimits();
            $product_ids = array_slice($product_ids,$limits[0],$limits[1]);

            echo "<span class='limits'>1:".$limits[0].' 2:'.$limits[1]."</span>";
            echo "</div>";
        }
		return $product_ids;
	}
...
function getOrderByList($virtuemart_category_id=false) {
	...
	$orderByList ='<div class="orderlistcontainer">Сортировать по:&nbsp;&nbsp;&nbsp;&nbsp;<a class="vmorder-link" title="'.$orderTxt.'" href="'.$link.'">'.JText::_('COM_VIRTUEMART_SEARCH_ORDER_'.$orderby).' '.$orderTxt.'</a>';
