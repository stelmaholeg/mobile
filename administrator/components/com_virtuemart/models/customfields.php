     ...
     function getProductCustomsChilds($childs){
		...
		$data = array();
	     	foreach ($childs as &$child) {
				...
				$query .=' and C.field_type = "C" ORDER BY C.ordering';
...
//		$result = $this->_db->loadAssocList();
	...
	public function displayCustomFields($datas){
        ...
        $html .= VmHTML::row('booleanlist','Краткая характеристика','is_on_common_tab',$datas->is_on_common_tab);
        $html .= VmHTML::row('booleanlist','Отображать в полном списке','is_on_fullinfo_tab',$datas->is_on_fullinfo_tab);

        $html .= VmHTML::row('booleanlist','Показывать в фильтрах','is_on_filterlist',$datas->is_on_filterlist);
        $html .= VmHTML::row('input','Порядок отображения в фильтрах','filter_order_num',$datas->filter_order_num);

        $html .= VmHTML::row('booleanlist','Является заголовком','is_group_name',$datas->is_group_name);
     ...
     */
     public function getproductCustomslist($virtuemart_product_id,$parent_id = null) {
		...
		$query='SELECT C.`virtuemart_custom_id` , `custom_element`, `custom_jplugin_id`, `custom_params`,
		               `custom_parent_id` , `admin_only` , `custom_title` , `custom_tip` , C.`custom_value` AS value,
		               `custom_field_desc` , `field_type` , `is_list` , `is_cart_attribute` , `is_hidden` ,
		               C.`is_on_common_tab` , C.`is_on_fullinfo_tab`, C.`is_on_filterlist`,C.`filter_order_num`,
		               C.`is_group_name` , C.`published` , field.`virtuemart_customfield_id`,
...
// 					return 'Automatic Childvariant creation (later you can choose here attributes to show, now product name) </td><td>';
    ...
    //$html .= VmHTML::row('booleanlist','is_group_name','is_group_name',$datas->is_group_name);
     public function getProductCustomsField($product) {

		$query='SELECT C.`virtuemart_custom_id` , `custom_element`, `custom_params`, `custom_parent_id` , `admin_only` ,
		               `is_on_common_tab`, `is_on_fullinfo_tab`, `is_on_filterlist`, `filter_order_num` , `is_group_name`, `custom_title` , `custom_tip` ,
		               C.`custom_value` AS value, `custom_field_desc` , `field_type` , `is_list` , `is_hidden`,
		               `layout_pos`, C.`published` , field.`virtuemart_customfield_id` , field.`custom_value`,
		               field.`custom_param`, field.`custom_price`, field.`ordering`
			FROM `#__virtuemart_customs` AS C
			LEFT JOIN `#__virtuemart_product_customfields` AS field
			  ON C.`virtuemart_custom_id` = field.`virtuemart_custom_id`
     ...
     public function getProductCustomsFieldRelatedCategories($product) {
		...
		$query='SELECT C.`virtuemart_custom_id` , `custom_parent_id` , `admin_only` , `custom_title` , `custom_tip` , C.`custom_value` AS value, `custom_field_desc` , `field_type` , `is_list` , `is_hidden` , `is_on_common_tab` ,`is_on_fullinfo_tab`, `is_on_filterlist`,`filter_order_num`, `is_group_name`, C.`published` , field.`virtuemart_customfield_id` , field.`custom_value`, field.`custom_param`, field.`custom_price`, field.`ordering`
			FROM `#__virtuemart_customs` AS C
     ...
     public function getProductCustomsFieldRelatedProducts($product) {
		...
		$query='SELECT C.`virtuemart_custom_id` ,
		               `custom_parent_id` , `admin_only` ,
		               `custom_title` , `custom_tip` ,
		               C.`custom_value` AS value, `custom_field_desc` ,
		               `field_type` , `is_list` , `is_hidden` ,
		               `is_on_common_tab` , `is_on_fullinfo_tab`, `is_on_filterlist`, `filter_order_num` ,`is_group_name`,
		               C.`published` , field.`virtuemart_customfield_id` , field.`custom_value`,
		               field.`custom_param`, field.`custom_price`, field.`ordering`
			FROM `#__virtuemart_customs` AS C
			LEFT JOIN `#__virtuemart_product_customfields` AS field ON C.`virtuemart_custom_id` = field.`virtuemart_custom_id`
